...
class LLSIR:
    ...
    def fit(self, x_0):
        ...
        if np.std(projections) < 1e-3:
            print("Warning: projections nearly constant; skipping KernelReg.")
            return beta, float(np.mean(Y_neighborhood)), float(self.f(self.gamma.project(x_0)[0]))
        else:
            nonparam_reg = KernelReg(endog=Y_neighborhood, exog=projections, var_type='c', bw=[1e-1])
        # get fitted values
